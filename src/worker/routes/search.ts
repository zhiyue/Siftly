...
async function getDbApiKey(db: ReturnType<typeof getDb>): Promise<string> {
...
async function getAllCategories(db: ReturnType<typeof getDb>) {
...
async function detectIntentCategories(db: ReturnType<typeof getDb>, query: string): Promise<string[]> {
...
function buildIndexEntry(b: {
  ...
  if (b.categories.length) {
    const cats = b.categories
      ...
      .map((cat) => `${cat.category.name}(${cat.confidence.toFixed(1)})`)
      .join(', ')
...
route.post('/api/search/ai', async (c) => {
  ...
  const apiKey = await getDbApiKey(db)
  ...
  let client: AIClient
  try {
    client = await resolveAIClient({ db, dbKey: apiKey, env })
  } catch {
    return c.json({ error: 'No API key configured. Add an API key in Settings.' }, 400)
  }
  const model = await getActiveModel(db)

  // Step 1: Smart candidate selection
  const keywords = extractKeywords(query)
  const intentSlugs = category ? [] : await detectIntentCategories(db, query)
  ...
  const ftsIds = keywords.length > 0 ? await ftsSearch(d1, keywords) : []
  ...
  if (allIds.length === 0) {
    return c.json({ bookmarks: [], explanation: 'No bookmarks found.' })
  }
...
- reason must be specific, not generic ("shows bitcoin price crash chart" not "related to crypto")`
  ...
  try {
    ...
    return c.json({ error: `AI search failed: ${errMsg}` }, 500)
  }
