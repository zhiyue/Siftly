...
// ── POST /api/import — file upload (FormData) ───────────────────────────────
...
// ── POST /api/import/twitter — live Twitter API fetch ────────────────────────
...
route.post('/api/import/twitter', async (c) => {
  ...
  if (!authToken?.trim() || !ct0?.trim()) {
    return c.json({ error: 'authToken and ct0 are required' }, 400)
  }

  if (source === 'like' && !userId) {
    return c.json({ error: 'userId is required for importing likes' }, 400)
  }
...
// ── POST /api/import/bookmarklet — accepts tweets from browser extension ─────

const ALLOWED_ORIGINS = new Set(['https://x.com', 'https://twitter.com'])

interface BookmarkletTweetResult {
  __typename?: string
  rest_id?: string
  legacy?: {
    full_text?: string
  ...
  note_tweet?: { note_tweet_results?: { result?: { text?: string } } }
  article?: { article_results?: { result?: ArticleResult } }
  tweet?: BookmarkletTweetResult
}

function bookmarkletTweetFullText(tweet: BookmarkletTweetResult): string {
  if (tweet.note_tweet?.note_tweet_results?.result?.text) {
...
function bookmarkletExtractMedia(tweet: BookmarkletTweetResult) {
  const entities =
    tweet.legacy?.extended_entities?.media ?? tweet.legacy?.entities?.media ?? []
  const results = entities
    .map((m) => {
...
route.options('/api/import/bookmarklet', (c) => {
...
route.post('/api/import/bookmarklet', async (c) => {
  const origin = c.req.header('Origin') ?? ''
  const allowed = ALLOWED_ORIGINS.has(origin) ? origin : 'https://x.com'
  const cors = {
    'Access-Control-Allow-Origin': allowed,
  ...
  let body: { tweets?: BookmarkletTweetResult[]; source?: string } = {}
  try {
    body = await c.req.json()
  } catch {
    return c.json({ error: 'Invalid JSON' }, { status: 400, headers: cors })
  }

  const source = body.source === 'like' ? 'like' : 'bookmark'
  const tweets = body.tweets ?? []
  if (!Array.isArray(tweets) || tweets.length === 0) {
    return c.json({ error: 'No tweets provided' }, { status: 400, headers: cors })
  }
  ...
  for (let tweet of tweets) {
    ...
    const userLegacy = tweet.core?.user_results?.result?.legacy ?? {}
    const media = bookmarkletExtractMedia(tweet)

    const created = await db
      .insert(bookmarks)
      .values({
        tweetId: tweet.rest_id,
        text: bookmarkletTweetFullText(tweet),
        authorHandle: userLegacy.screen_name ?? 'unknown',
        authorName: userLegacy.name ?? 'Unknown',
        tweetCreatedAt: tweet.legacy?.created_at
          ? new Date(tweet.legacy.created_at).toISOString()
  ...
  return c.json({ imported, skipped }, { headers: cors })
})
